...
class bLSTM(object):
	...
	def __init__(self, input_seq_length, output_seq_length, input_dict_size, num_classes, input_embed_size, output_embed_size, num_layers, num_LSTM_cells, batch_size, 
		...
		self.alternative_targets = tf.placeholder(tf.int8, (None,None),'alternative_targets') # Orthographically incorrect, but accepted spellings: bs x seq_len x max_alt_targs
	...
	def backward(self):
		...
		with tf.name_scope("optimization_"+ self.task):
			...
			if self.learn_type == 'normal':
				...
				self.loss, self.read_inps = tf.contrib.seq2seq.sequence_loss_lds(self.logits, self.targets, 
